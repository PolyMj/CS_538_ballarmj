...
PotatoRaytracerEngine::PotatoRaytracerEngine(int windowWidth, int windowHeight) : PotatoRenderEngine(windowWidth, windowHeight) {
	...
	Mat4d flrMat = translate(0.0, -1.0, 0.0) * scale(1000.0, 0.1, 1000.0);
	...
	floor->transform(flrMat);
	meshes.push_back(floor);


	Mat4d fox1Mat = translate(0.0, 0.2, -3.0) * vaseMat;
	...
	fox->transform(fox1Mat);
	meshes.push_back(fox);

	Mat4d wall1Mat = translate(0.0, 1.0, -7.0) * rotateMat(0.0, 1.2, 0.0) * scale(10.0, 10.0, 0.1);
	...
	wall1->transform(wall1Mat);
	meshes.push_back(wall1);

	Mat4d wall2Mat = translate(0.0, 1.0, -7.0) * rotateMat(0.0, -1.2, 0.0) * scale(10.0, 10.0, 0.1);
